...
### 2026-08-12: Messages sent to wrong conversation after switching chats — URL↔State race condition

- **Symptom**: User clicks Chat B from sidebar → UI correctly loads Chat B's messages → user sends a message → message is created in Chat A (the previous chat). Occurs frequently after 30+ minutes of inactivity.
- **Root Cause**: Race condition in `useUrlSync.ts`. The URL→State `useEffect` had `selectedConversationId` in its dependency array. When `setSelectedConversationIdAndUrl("B")` was called: (1) state updated synchronously to "B", (2) `router.push("?id=B")` ran asynchronously. The state change triggered the effect, which read stale `searchParams` (still "A") and reverted state back to "A". After idle periods, Next.js RSC cache expiry made `router.push()` slower, widening the race window.
- **Fix**: Removed `selectedConversationId` from the useEffect dependency array. Used `useRef` to read current state inside the effect without triggering it. Effect now only fires when `searchParams` actually changes (browser back/forward, page reload).
- **Prevention Rule**: **In bidirectional sync hooks (URL ↔ State), the "external → internal" sync effect must NOT include the internal state in its dependency array.** Use a `useRef` to read the current internal state for comparison. Otherwise, programmatic internal updates trigger the effect which reads stale external state and reverts the change. This is especially dangerous with Next.js `router.push()` which updates `searchParams` asynchronously.
...
### 2026-07-01: Deep Research UI stuck at "Analyzing results" ⚠️ RECURRING

- **Symptom**: Deep Research UI shows "Analyzing results" indefinitely while the backend is still running and polling correctly. After clicking "Start Research", the progress card immediately jumps to "Analyzing results" active state instead of showing "Initializing agent..." during the startup phase.
- **Root Cause**: The `getResearchInteraction` parser correctly deduces `currentStep` based on step types, and `ResearchProgressCard` has built-in "Initializing agent..." UI for when `currentStep` is undefined. However, `ChatApp.tsx` passed `currentStep={currentTask.currentStep || "analyzing"}` — the `|| "analyzing"` fallback overrode `undefined` with a hardcoded value, preventing the initialization UI from ever appearing.
- **Fix (original 2026-07-01)**: Added step-deduction logic in `getResearchInteraction` and set `task.currentStep` in polling.
- **Fix (regression 2026-07-03)**: Removed `|| "analyzing"` fallback in `ChatApp.tsx` for both `planning` and `executing` phases. Now passes `currentTask.currentStep` directly, allowing `ResearchProgressCard` to correctly show "Initializing agent..." when the step is undefined.
- **Prevention Rule**: **Never override backend-provided state with UI-side fallbacks that mask "unknown/pending" states.** If a component already handles `undefined` gracefully (e.g., showing an initializing indicator), the parent must NOT replace `undefined` with a default value. The `|| "default"` pattern is dangerous for progress indicators — it hides the distinction between "not started" and "actively running".
...
### 2026-07-03: Deep Research duplicate SSE streams causing double stale detection and early failure

- **Symptom**: Backend logs showed 2 parallel SSE poll loops for the same task ID after clicking "Start Research" (approve). Both triggered stale detection independently, exhausting retries twice as fast → task failed after ~7 minutes with "Gemini silent hang" message.
- **Root Cause**: The SSE stream route's terminal status check only recognized `completed` and `failed`. When planning finished and the task status changed to `ready_to_execute`, SSE stream #1 (from planning) did NOT close — it kept polling indefinitely. When the user approved the plan, the client opened SSE stream #2 (for execution). Both streams polled the same exec interaction ID, both triggered stale detection, both created retry interactions — racing each other and exhausting `MAX_STALE_RETRIES` (3) twice as fast.
