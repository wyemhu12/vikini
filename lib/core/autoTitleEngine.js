...
function extractText(resp) {
  try {
    ...
    console.error("[AutoTitle] Error extracting text:", e);
  }
  return "";
}
...
async function generateTitleWithModel(promptText, { temperature, maxOutputTokens }) {
  ...
  try {
    ...
    console.warn(`[AutoTitle] Primary model ${PRIMARY_MODEL} failed, trying fallback. Error: ${e1.message}`);
    
    try {
      // Thử model dự phòng (Gemini 2.5 Flash Stable)
      return await attemptGenerate(FALLBACK_MODEL);
    } catch (e2) {
      console.error(`[AutoTitle] All models failed. Fallback: ${e2.message}`);
      return null;
    }
  }
}
