...
/**
 * Get token limit for a specific model ID.
 * Returns a safe default if not found.
 */
export function getModelTokenLimit(modelId) {
  const id = coerceStoredModel(modelId);
  const model = SELECTABLE_MODELS.find((m) => m.id === id);
  // Default to 30k tokens (~120k chars) if unknown, which is safe for most modern models
  return model?.tokenLimit || 32000;
}
