...
// ... (Giữ nguyên các hàm helper isOfficeDocMime, isZipMime, v.v...)
...
export async function handleChatStreamCore({ req, userId }) {
  ...
  // --- LOGIC TỐI ƯU ---
  // Xác định xem có cần tạo title không:
  // 1. Hội thoại mới tạo (createdConversation)
  // 2. HOẶC hội thoại cũ nhưng tên vẫn là "New Chat" (do lần trước lỗi)
  // 3. VÀ không phải đang regenerate (trừ khi nó chưa có title)
  ...
  // Attachments logic ... (Giữ nguyên phần attachments dài dòng để đảm bảo file đầy đủ)
  try {
  ...
  const stream = createChatReadableStream({
    ...
    createdConversation,
    shouldGenerateTitle, // TRUYỀN FLAG NÀY SANG STREAMING
    enableWebSearch,
