"use client";

import { signIn } from "next-auth/react";
import { useLanguage } from "../../features/chat/hooks/useLanguage";
import { useTheme } from "../../features/chat/hooks/useTheme";

export default function SignInPage() {
  const { t, language, setLanguage } = useLanguage();
  const { theme } = useTheme();

  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-neutral-950 p-4 text-white">
      <div className="w-full max-w-sm space-y-8 text-center">
        {/* Logo/Title Area */}
        <div className="space-y-2">
          <h1 className="text-4xl font-bold tracking-tighter text-[var(--primary)]">
            {t("appName") || "Vikini"}
          </h1>
          <p className="text-sm font-medium uppercase tracking-widest text-neutral-500">
            {t("whitelistOnly") || "White Listed Only"}
          </p>
        </div>

        {/* Action Area */}
        <div className="rounded-2xl border border-neutral-800 bg-neutral-900/50 p-8 shadow-2xl backdrop-blur-sm">
          <p className="mb-6 text-sm text-neutral-400">
            {language === "vi" 
              ? "Vui lòng đăng nhập để tiếp tục trải nghiệm." 
              : "Please sign in to continue your experience."}
          </p>

          <button
            onClick={() => signIn("google", { callbackUrl: "/" })}
            className="flex w-full items-center justify-center gap-3 rounded-xl bg-white px-6 py-3 text-sm font-semibold text-black transition-all hover:bg-neutral-200 active:scale-95"
          >
            <svg className="h-5 w-5" viewBox="0 0 24 24">
              <path
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                fill="#4285F4"
              />
              <path
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                fill="#34A853"
              />
              <path
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                fill="#FBBC05"
              />
              <path
                d="M12 5.38c1.62 0 3.06.56 4.21 1.66l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                fill="#EA4335"
              />
            </svg>
            {language === "vi" ? "Tiếp tục với Google" : "Continue with Google"}
          </button>
        </div>

        {/* Language Switcher for Guest */}
        <div className="flex items-center justify-center gap-4 text-xs font-medium text-neutral-500">
          <button
            onClick={() => setLanguage("vi")}
            className={`transition-colors hover:text-white ${language === "vi" ? "text-[var(--primary)]" : ""}`}
          >
            TIẾNG VIỆT
          </button>
          <span className="h-3 w-px bg-neutral-800"></span>
          <button
            onClick={() => setLanguage("en")}
            className={`transition-colors hover:text-white ${language === "en" ? "text-[var(--primary)]" : ""}`}
          >
            ENGLISH
          </button>
        </div>
      </div>
    </div>
  );
}
